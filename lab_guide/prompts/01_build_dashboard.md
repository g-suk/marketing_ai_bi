...
2. **Advanced Analytics** — Four tabs: "Marketing Mix Model", "Multi-Touch Attribution", "Geo-Targeting", "CLV & Churn". Use direct SQL queries (not SEMANTIC_VIEW) against these tables. Each tab should include a collapsible `st.expander()` at the top explaining the methodology (how the score/model works, what the data pipeline is). Keep expanders concise — a short paragraph plus a few bullet points max. Avoid tables or lengthy breakdowns inside expanders; they should be scannable in 5 seconds.
   ...
   **Tab 2 - Multi-Touch Attribution:** Three journey KPI metric cards at top (Total Conversions, Avg Touchpoints/Journey, Avg Days to Conversion) from `DT_MTA_JOURNEY_SUMMARY` (columns: conversion_id, customer_id, revenue, total_touchpoints, journey_duration_days, channel_path). Grouped bar chart comparing all 5 attribution models side-by-side from `V_CHANNEL_ATTRIBUTION_SUMMARY` (columns: sub_channel, model_name, total_attributed_revenue, conversions_attributed) — x-axis = sub_channel, y-axis = attributed revenue, color = model_name, use `xOffset` for grouped bars. Single-model selector with horizontal bar chart breakdown. Consensus ranking table: rank each channel within each model by attributed revenue, then average ranks across models (lower avg rank = stronger channel). The 5 models are: first_touch, last_touch, linear, time_decay, position_based.
